...
// Helpers to make pendingNetworkInvites fork-aware (Call-ID + top Via branch)
function getBranch(sipMsg) {
  const via = sipMsg.get('via');
  if (!via) return null;
  const match = via.match(/;branch=([^;]+)/i);
  return match && match[1] ? match[1] : null;
}

function makeBranchKey(callId, branch) {
  return branch ? `${callId}|${branch}` : callId;
}

function addPendingNetworkInvite(obj, sipMsg, invite) {
  const callId = sipMsg.get('call-id');
  const branch = getBranch(sipMsg);
  const key = makeBranchKey(callId, branch);
  obj.pendingNetworkInvites.set(key, Object.assign({ callId }, invite));
}

function getPendingNetworkInvite(obj, sipMsg) {
  const callId = sipMsg.get('call-id');
  const branch = getBranch(sipMsg);
  const key = makeBranchKey(callId, branch);
  if (obj.pendingNetworkInvites.has(key)) return obj.pendingNetworkInvites.get(key);

  // backwards-compatibility: fall back to call-id key if present
  return obj.pendingNetworkInvites.get(callId);
}

function deletePendingNetworkInvite(obj, sipMsg) {
  const callId = sipMsg.get('call-id');
  const branch = getBranch(sipMsg);
  const key = makeBranchKey(callId, branch);
  if (obj.pendingNetworkInvites.delete(key)) return;

  // fallback: legacy key
  obj.pendingNetworkInvites.delete(callId);
}

function deletePendingNetworkInvitesByCallId(obj, callId) {
  for (const [key, value] of obj.pendingNetworkInvites) {
    if ((value && value.callId && value.callId === callId) || key === callId) {
      obj.pendingNetworkInvites.delete(key);
    }
  }
}
...
class DrachtioAgent extends Emitter {
  ...
  sendResponse(res, opts, callback, fnAck) {
    ...
    if (res.statusCode >= 200) {
      ...
      if (res.req.method === 'INVITE') {
        ...
        deletePendingNetworkInvite(obj, res.req);
  ...
  proxy(req, opts, callback) {
    ...
    deletePendingNetworkInvite(obj, req);
    const callId = req.get('Call-Id') || req.get('call-id');
    debug(`proxying call, deleted pending invite for call-id ${callId}, ` +
      `pendingNetworkInvites size is now ${obj.pendingNetworkInvites.size}`);
  }
  ...
  _initServer(socket) {
    ...
    this.mapServer.set(socket, {
      ...
      // key: Call-ID|branch or Call-ID (legacy)
        ...
        if (token.length > 9) {
          ...
          if ('network' === source && sipMsg.type === 'request') {
            ...
            if ('CANCEL' === sipMsg.method) {
              ...
              const pending = getPendingNetworkInvite(obj, sipMsg);
              if (pending) {
                pending.req.emit('cancel', sipMsg);
                deletePendingNetworkInvite(obj, sipMsg);
            ...
            if ('INVITE' === req.method) {
              addPendingNetworkInvite(obj, sipMsg, { req, res });
            ...
            else if ('UPDATE' === req.method && getPendingNetworkInvite(obj, sipMsg)) {
              const inviteReq = getPendingNetworkInvite(obj, sipMsg).req;
